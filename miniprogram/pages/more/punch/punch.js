...
Page({
  ...
  requestCode: function(){
    var that = this;
    wx.request({
      url: "https://wa.gdupt.edu.cn:8080/signInQRCode/"+ that.data.taskId  + "/getQRCodeImg",
      method: 'get',
      header: {
        'loginUserTime': that.data.token
      },
      success: function (res1) {
        console.log(res1)
        var base64 = res1.data.data.data.qrCodeImg;
        var aaa = wx.getFileSystemManager();
        that.setData({
          'imgData': base64
        })
        wx.showToast({
  ...
  cx: function (e) {
    ...
    if(that.prefixInteger(that.data.arr[that.data.index]) === this.data.code && that.data.taskId !== ""){
      that.requestCode()
      return
    }
    wx.cloud.callFunction({
      ...
      success: res => {
        if (res.result.msg == "教室获取失败") {
          wx.showToast({
            icon: 'none',
            title: '教室获取失败,确定是否有这个课',
          })
          return
        } else if (res.result.msg == "关闭") {
          wx.showToast({
            icon: 'none',
            title: '关闭了',
          })
          ...
          that.data.taskId = res.result.taskId
          that.data.code = that.prefixInteger(that.data.arr[that.data.index])
          that.data.token = res.result.msg
          that.requestCode()
        }
      },
      fail: err => {
